...
// randomInt (CSPRNG): estos códigos y contraseñas protegen las bases de datos
// de pacientes — Math.random() sería predecible.
...
function generatePassword(length = 20): string {
  ...
  let pwd = '';
  for (let i = 0; i < length; i++) pwd += chars[randomInt(chars.length)];
  return pwd;
}
